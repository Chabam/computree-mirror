...
void PolygonForPicking::drawOverlay(QPainter &painter, DrawInfo &info)
{
    Q_UNUSED(info)
    ...
    painter.save();
        ...
        drawPolygon(m_polygon, painter, info, false);

        // draw the preview
        drawPreview(painter, info);
...
void PolygonForPicking::drawPolygon(const QPolygon &pol, QPainter &painter, DrawInfo &info, bool triangulated)
{
    if(triangulated)
        drawPolygonTriangulated(pol, painter, info);
    else {
        if(m_polygonClosed) {
...
void PolygonForPicking::drawPolygonTriangulated(const QPolygon &pol, QPainter &painter, DrawInfo &info)
{
    Q_UNUSED(info)

    painter.setPen(createPenToUse(Qt::yellow));
    QVector<QPolygon> triangles = ShapeToVolume::staticConvertPolygonToTriangle(pol);

    foreach (const QPolygon &triangle, triangles) {
        drawPolygon(triangle, painter, info, false);
    }
}

void PolygonForPicking::drawInterestPoint(QPainter &painter, DrawInfo &info)
{
    Q_UNUSED(info)
...
void PolygonForPicking::drawPreview(QPainter &painter, DrawInfo &info)
{
    Q_UNUSED(info)
