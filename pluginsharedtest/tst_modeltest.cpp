...
/*void ModelTest::testCaseNewInTreeCreation()
{
    CT_UniqueIndexGenerator generator;
    CT_InModelConstructor constructor(generator);

    CT_InResultModelHelper result = constructor.addResult("DIR", "DER", true);
    CT_InGroupModelHelper<> root = constructor.setAZeroOrMoreRootGroup(result);
    CT_InGroupModelHelper<CT_TOPFNodeGroup> group = constructor.addGroup<CT_TOPFNodeGroup>(root, "DIG", "DEG", CT_InAbstractGroupModel::CG_ChooseOneIfMultiple, CT_InAbstractGroupModel::FG_IsObligatory);
    CT_InItemModelHelper<> item = constructor.addItem<>(group, "DI", "DE", CT_InStdSingularItemModel::C_ChooseOneIfMultiple, CT_InStdSingularItemModel::F_IsOptional);
