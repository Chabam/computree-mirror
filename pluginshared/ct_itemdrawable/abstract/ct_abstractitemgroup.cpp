...
QVector<CT_AbstractItem *> CT_AbstractItemGroup::childrensForGui() const
{
    ...
    while(itG.hasNext())
    {
        QList<CT_AbstractItemDrawable*>* itemsInGroup = itG.next().value()->getList();

        r.resize(currentIndex + itemsInGroup->size());

        QListIterator<CT_AbstractItemDrawable*> itL(*itemsInGroup);

        while(itL.hasNext())
            r[currentIndex++] = itL.next();
    }
