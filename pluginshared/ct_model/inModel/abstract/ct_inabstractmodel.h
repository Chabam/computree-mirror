...
public:
    ...
    /**
     * @brief Visit all possibilities of all input result models
     * @param visitor : the visitor to use
     */
    void visitPossibilities(std::function<void(CT_InAbstractModel* model, CT_InStdModelPossibility* possibility)>& visitor);

    /**
     * @brief Find one possibility in the out model for this model and its children
     * @param model : the OUTPUT model where begin the search
     ...
     * @return true is the test pass.
     */
    virtual bool isAtLeastOnePossibilitySelectedIfItDoes(QStringList* errors = NULL) const;
     ...
     * @return true is the test pass.
     */
    virtual bool recursiveIsAtLeastOnePossibilitySelectedIfItDoes(QStringList* errors = NULL) const;
