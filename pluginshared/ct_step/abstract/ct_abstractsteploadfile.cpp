...
CT_AbstractStepLoadFile::CT_AbstractStepLoadFile(CT_StepInitializeData &dataInit) : SuperClass(dataInit)
{
    _filePath = "";
    _defaultDirPath = ".";
}
...
void CT_AbstractStepLoadFile::savePreSettings(SettingsWriterInterface &writer) const
{
    ...
    writer.addParameterPath(this, "FilePath", _filePath);
}
...
bool CT_AbstractStepLoadFile::restorePreSettings(SettingsReaderInterface &reader)
{
    ...
    reader.parameterPath(this, "FilePath", _filePath);
...
{
    if(!isRunning() && SuperClass::postConfigure())
    {
        QString fp = getFilePath();

        if(fp.isEmpty())
            fp = getDefaultDirPath();

        QString s = QFileDialog::getOpenFileName(NULL, getStepDescription(), fp, tr("%1 compatible file (%2);;All Files (*.*)").arg(getStepDisplayableName()).arg(createAcceptedExtensionString(" *")));
