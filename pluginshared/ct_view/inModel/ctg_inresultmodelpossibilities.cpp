...
{
    m_inputResultModelManager = (CT_InResultModelManager*)manager;
...
void CTG_InResultModelPossibilities::constructModel()
{
    ...
    if(m_inputResultModelManager != NULL)
    {
        ...
        for(const CT_InAbstractResultModel* inputResultModel : inputResultModels) {
            const QList<QStandardItem*> items = createItemsForResultModel(inputResultModel);
...
QList<QStandardItem*> CTG_InResultModelPossibilities::createItemsForResultModel(const CT_InAbstractResultModel* resModel) const
{
    ...
    rootName->setData(resModel->description(), Qt::UserRole + 2);               // the description (help) is set on the first column
    ...
    for(const CT_InStdModelPossibility* possibility : possibilities)
    {
        ...
        item = new QStandardItem(possibility->outModel()->step()->getStepCustomName());
        item->setDragEnabled(false);
        item->setEditable(false);
        item->setData(qVariantFromValue((void*)possibility), Qt::UserRole+1); // pointer to the possibility
        rowList.append(item);
