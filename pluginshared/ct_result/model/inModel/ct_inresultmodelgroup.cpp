...
CT_InResultModelGroup::CT_InResultModelGroup(const QString &uniqueName,
                                             ...
                                             bool recursive) : SuperClass(uniqueName,
...
void CT_InResultModelGroup::saveSettings(SettingsWriterInterface &writer) const
{
    ...
    writer.addParameter(this, "GroupMiniNPSelect", possibilitiesGroup()->minimumNumberOfPossibilityThatMustBeSelected());
    writer.addParameter(this, "GroupMaxiNPSelect", possibilitiesGroup()->maximumNumberOfPossibilityThatCanBeSelected());
...
bool CT_InResultModelGroup::restoreSettings(SettingsReaderInterface &reader)
{
    ...
    if(!reader.parameter(this, "GroupMiniNPSelect", value) || (value.toInt() != possibilitiesGroup()->minimumNumberOfPossibilityThatMustBeSelected()))
        return false;

    if(!reader.parameter(this, "GroupMaxiNPSelect", value) || (value.toInt() != possibilitiesGroup()->maximumNumberOfPossibilityThatCanBeSelected()))
        return false;
              ...
              && (sp != NULL))
        {
            ...
            if(sp != NULL)
            {
                ...
                for(const CT_OutAbstractResultModel* outputResultModel : outputResultModels)
                    recursiveFindAllPossibilitiesInModel(*outputResultModel, true);
...
bool CT_InResultModelGroup::canBeComparedWith(const CT_OutAbstractModel &model) const
{
    ...
    return (dynamic_cast<const CT_OutAbstractResultModel*>(&model) != NULL);
}
                                                                     ...
                                                                     const CT_OutAbstractResultModel *model) const
{
    ...
    QList<CT_ResultGroup*> list = step->getOutResultList();
    ...
    while(it.hasNext())
    {
        ...
        // compare uniqueName for result model (only result model have a turn)
        if(res->model()->uniqueName() == model->uniqueName())
            return res;
    }
