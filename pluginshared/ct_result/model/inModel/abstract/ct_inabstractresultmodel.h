...
public:
     ...
     */
    bool recursiveIsAtLeastOnePossibilitySelectedIfItDoes(QStringList* errors = NULL) const override;
    ...
    /**
     * @brief A model that represent a result is compared to each OUTPUT model (result) of a step (for multiple step). Childrens must not mix elements
...
protected:
     ...
     *           is compared for each OUTPUT model (result) of a step of multiple step. Childrens must not mix elements of one
