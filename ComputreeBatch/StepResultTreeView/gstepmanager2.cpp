...
GStepManager2::GStepManager2(QWidget *parent) :
    m_stepManager(*static_cast<BatchDialog*>(parent)->getBatchCore().getStepManager()),
    QWidget(parent),
    ui(new Ui::GStepManager2)
{
    ui->setupUi(this);

    setStepNameConfiguration(DNC_StepShortDescription);
    ...
    connect(&m_stepManager, SIGNAL(stepAdded(CT_VirtualAbstractStep*)), this, SLOT(stepAdded(CT_VirtualAbstractStep*)), Qt::QueuedConnection);
    connect(&m_stepManager, SIGNAL(stepInserted(int,CT_VirtualAbstractStep*)), this, SLOT(stepInserted(int,CT_VirtualAbstractStep*)), Qt::QueuedConnection);
    connect(&m_stepManager, SIGNAL(stepToBeRemoved(CT_VirtualAbstractStep*)), this, SLOT(stepToBeRemoved(CT_VirtualAbstractStep*)), Qt::DirectConnection);
...
QString GStepManager2::staticGetStepNameFromConfiguration(CT_VirtualAbstractStep *step, const DisplayNameConfigs &config, bool showStepID)
{
    QString name = "";
    ...
    if (showStepID)
        name = QString(" %1 -").arg(step->uniqueID()) + name;

    return name;
}

void GStepManager2::setStepNameConfiguration(DisplayNameConfigs config)
{
    ...
    if(extractResultInformationFromTree(child, name, isClearedFromMemory, isBusy)) {
        ...
        for(int i=0; i<ui->treeWidget->columnCount(); ++i) {
            if(isClearedFromMemory)
                child->setBackground(i, QColor(236, 208, 209));
            else
                child->setBackground(i, QColor(220, 220, 220));
        }
...
{
    for(int i=0; i<ui->treeWidget->columnCount(); ++i) {
        child->setBackground(i, isManual ? QColor(183, 225, 170) : QColor(170, 186, 225));
    }
}
