...
/**
 * @brief Use this class to modify all object of a specified type in a scene
 */
class DM_MultipleObjectsModifier : public ObjectsModifierInterface
{
public:
    DM_MultipleObjectsModifier(const PermanentItemSceneByModel *scene,
...
private:
    typedef QVector<IChunk*>            ChunkCollection;
    typedef QVectorIterator<IChunk*>    ChunkCollectionIterator;
    ...
    /**
     * @brief Find the chunk for the specified object index (use and change m_lastChunk to speed up the process)
     * @param objectIndex : object index to found
     * @param chunkLocalObjectIndex (out) : local index in the chunk cloud of the object index
     */
    IChunk* findChunkForObjectIndex(const size_t& objectIndex, size_t& chunkLocalObjectIndex);
};

#endif // DM_MULTIPLEOBJECTSMODIFIER_H
