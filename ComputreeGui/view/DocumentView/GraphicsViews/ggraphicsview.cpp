...
void GGraphicsView::setCurrentInternalAction(CT_AbstractActionForGraphicsView *action)
{
    if(action != nullptr)
    {
        action->setDocument(getDocumentView());
    ...
    m_actionsHandler->setCurrentInternalAction(action);
}
