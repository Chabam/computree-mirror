...
void GMainWindow::exportDocumentedScript()
{
    ...
    CDM_StepListDocExporter::DocumentatedScriptInfo docInfo;
    ...
    if(mainDir.exists())
    {
        ...
        if (dialog.exec())
        {
            dialog.getInfo(docInfo);

            QString outDirPath = mainDirPath + "/" + docInfo._scriptFileName;

            QDir outDir(outDirPath);
            if (!outDir.exists()) {mainDir.mkdir(docInfo._scriptFileName);}
            ...
            QDesktopServices::openUrl(QUrl("file:///" + outDirPath + "/" + docInfo._scriptFileName + ".html"));
        }
    }
}
...
void GMainWindow::initUI()
{
    ...
    actionExportDocumentedScript = new QAction(tr("Exporter script documenté (dont citations)"), this);
    actionExportDocumentedScript->setIcon(QIcon(":/Icones/Icones/citation.png"));
    ...
    ui->toolBar->addAction(actionExportDocumentedScript);
    ...
    ui->menuFichier->addAction(actionExportDocumentedScript);
    ui->menuFichier->addSeparator();
    ...
    actionExportDocumentedScript->setEnabled(true);
    ...
    connect(actionExportDocumentedScript, SIGNAL(triggered()), this, SLOT(exportDocumentedScript()));
    ...
    connect(getStepManager(), SIGNAL(stepFinishExecuted(CT_VirtualAbstractStep*)), _docManagerView, SLOT(stepFinished(CT_VirtualAbstractStep*)), Qt::BlockingQueuedConnection);
