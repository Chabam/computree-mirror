    ...
    if(pluginManager->isAPluginLoaded())
    {
        ...
        if(stepLfList.size() > 1)
        {
            ...
            while(itLf.hasNext())
            {
                ...
                cb->addItem(GStepViewDefault::staticGetStepNameFromConfiguration(stepLf, m_stepNameConfig, true) + " (" + pluginManager->getPluginName(stepLf->getPlugin()) + ")");
            }
    ...
    if(result != NULL) {
        if(checked) {
            ...
            if(GUI_MANAGER->removeEditItemDrawableModelOfResult(*result))
                GUI_MANAGER->asyncRemoveAllItemDrawableOfResultFromView(*result, NULL);
        }
    }
}
