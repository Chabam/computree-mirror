...
bool CT_Reader_LAZ::internalReadFile(CT_StandardItemGroup *group)
{
    ...
    if (laszip_create(&laszip_reader) == 0)
    {
        ...
        if (laszip_open_reader(laszip_reader, filepath.toStdString().c_str(), &is_compressed) == 0)
        {
            ...
            if (laszip_get_point_pointer(laszip_reader, &point) == 0)
            {
                ...
                if (filterSet())
                {
                    ...
                    for (size_t i = 0; i < nPoints; ++i)
                    {
                        ...
                        {
                            PS_LOG->addErrorMessage(LogInterface::reader, tr("Impossible de lire le point (laszip_read_point)"));
                        } else
                        {
                            x = point->X;
                            y = point->Y;
            ...
            {
                PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadFile : impossible de fermer le fichier"));
            }

        } else {
            valid = false;
            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadFile : impossible d'ouvrir le fichier"));
        }

        // destroy the reader
        if (laszip_destroy(laszip_reader))
        {
            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadFile : impossible de libérer l'accès au fichier"));
        }
    } else {
        valid = false;
        PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadFile : impossible d'accéder au fichier)"));
    }
            ...
            if (laszip_open_reader(laszip_reader, filepath.toStdString().c_str(), &is_compressed) == 0)
            {
                ...
                if (laszip_get_point_pointer(laszip_reader, &point) == 0)
                {
                    for(const qint64 &i: fileBuffer.indexList)
                    {
                        ...
                        if (laszip_read_point(laszip_reader))
                        {
                            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : Impossible de lire le point"));
                        } else
                        {
                            // READ ALL ATTRIBUTES
                            qint32 x = point->X;
                    ...
                    valid = false;
                    PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : Impossible de lire les points"));
                }

                // close the reader
                if (laszip_close_reader(laszip_reader))
                {
                    PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : impossible de fermer le fichier"));
                }

            } else {
                valid = false;
                PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : impossible d'ouvrir le fichier"));
            }

            // destroy the reader
            if (laszip_destroy(laszip_reader))
            {
                PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : impossible de libérer l'accès au fichier"));
            }
        } else {
            valid = false;
            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::internalReadMultiFile : impossible d'accéder au fichier)"));
        }
    }
        ...
        if (laszip_open_reader(laszip_reader, filepath.toStdString().c_str(), &is_compressed) == 0)
        {
            ...
            {                
                ...
                for (qint64 i = 0; i < nPoints; ++i)
                {
                    ...
                    if (laszip_read_point(laszip_reader))
                    {
                        PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : Impossible de lire le point"));
                    } else
                    {
                        x = point->X;
                        y = point->Y;
                ...
                PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : Impossible de lire les points"));
            }

            // close the reader
            if (laszip_close_reader(laszip_reader))
            {
                PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : impossible de fermer le fichier"));
            }

        } else {
            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : impossible d'ouvrir le fichier"));
        }

        // destroy the reader
        if (laszip_destroy(laszip_reader))
        {
            PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : impossible de libérer l'accès au fichier"));
        }
    } else {
        PS_LOG->addErrorMessage(LogInterface::reader, tr("CT_Reader_LAZ::getPointIndicesInside2DShape : impossible d'accéder au fichier)"));
    }
