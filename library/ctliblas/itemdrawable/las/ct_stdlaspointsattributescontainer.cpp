...
bool CT_StdLASPointsAttributesContainer::getLASDataAt(const size_t i, CT_LASData &data) const
{
    ...
    if (((attribute = pointsAttributesAt(CT_LasDefine::GPS_Time)) != nullptr) && attribute->hasBeenSet(i)) { atLeastHasOneData = true; data._GPS_Time = quint16(static_cast<CT_AbstractPointAttributesScalar*>(attribute)->scalarAsDoubleAt(i));} else {data._GPS_Time = 0;}
    ...
    if (((attribute = pointsAttributesAt(CT_LasDefine::Byte_offset_to_waveform_data)) != nullptr) && attribute->hasBeenSet(i)) { atLeastHasOneData = true; data._Byte_offset_to_waveform_data = quint16(static_cast<CT_AbstractPointAttributesScalar*>(attribute)->scalarAsDoubleAt(i));} else {data._Byte_offset_to_waveform_data = 0;}
    if (((attribute = pointsAttributesAt(CT_LasDefine::Waveform_packet_size_in_bytes)) != nullptr) && attribute->hasBeenSet(i)) { atLeastHasOneData = true; data._Waveform_packet_size_in_bytes = quint16(static_cast<CT_AbstractPointAttributesScalar*>(attribute)->scalarAsDoubleAt(i));} else {data._Waveform_packet_size_in_bytes = 0;}
    if (((attribute = pointsAttributesAt(CT_LasDefine::Return_Point_Waveform_Location)) != nullptr) && attribute->hasBeenSet(i)) { atLeastHasOneData = true; data._Return_Point_Waveform_Location = quint16(static_cast<CT_AbstractPointAttributesScalar*>(attribute)->scalarAsDoubleAt(i));} else {data._Return_Point_Waveform_Location = 0;}
