...
bool CT_Reader_LAS::restoreSettings(SettingsReaderInterface &reader)
{
    delete m_headerFromConfiguration;
    m_headerFromConfiguration = NULL;
...
void CT_Reader_LAS::staticReadCurrentPointCoordinateAndArraysOfDatas(QDataStream &stream, const CT_LASHeader &header, qint32 &x, qint32 &y, qint32 &z, std::vector<char> &datas)
{
    stream >> x >> y >> z;

    datas.resize(header.get_pointDataRecordLength() - 12); // 12 = (3 * qint32) [x, y, z]

    stream.readRawData(&datas[0], int(datas.size()));
}

void CT_Reader_LAS::staticReadCurrentPoint(QDataStream &stream,
                                           qint32 &x, qint32 &y, qint32 &z,
                                           quint16 &intensity,
                                           quint8* mask05, quint16* mask610,
                                           quint8 &classification,
                                           qint8 *scanAngleRank05, qint16 *scanAngleRank610,
                                           quint8 &userData,
                                           quint16 &pointSourceID,
                                           double *gpsTime,
                                           quint16 *red, quint16 *green, quint16 *blue,
                                           quint16 *nir,
                                           quint8 *wpdi, quint64 *botwd, quint32 *wpsib, float *rpwl)
{
    stream >> x >> y >> z >> intensity;
    ...
    if(scanAngleRank610 == NULL)
    {
        stream >> (*scanAngleRank05);
        stream >> userData;
    }
    else
    {
...
bool CT_Reader_LAS::protectedReadFile()
{
    ...
    if(f.open(QIODevice::ReadOnly))
    {
        ...
        quint8* mask05 = NULL;
        quint16* mask610 = NULL;
        qint8 *scanAngleRank05 = NULL;
        qint16 *scanAngleRank610 = NULL;
        double *gpsTime = NULL;
        quint16 *red = NULL;
        quint16 *green = NULL;
        quint16 *blue = NULL;
        ...
        for(size_t i=0; i<nPoints; ++i)
        {
            if(mask6_10V == NULL) {
                mask610 = NULL;
                mask05 = &mask0_5V->tAt(i).entire;
            } else {
                mask05 = NULL;
                mask610 = &mask6_10V->tAt(i).entire;
            }

            if(scanAngleRankV6_10 == NULL) {
                scanAngleRank610 = NULL;
                scanAngleRank05 = &scanAngleRankV0_5->tAt(i);
            } else {
                scanAngleRank05 = NULL;
                scanAngleRank610 = &scanAngleRankV6_10->tAt(i);
            }

            gpsTime = ((gpsTimeV != NULL) ? &gpsTimeV->tAt(i) : NULL);

            if(colorsRV != NULL)
            {
                red = &colorsRV->tAt(i);
                green = &colorsGV->tAt(i);
                blue = &colorsBV->tAt(i);
            } else {
                red = NULL;
            }

            nir = ((nirV != NULL) ? &nirV->tAt(i) : NULL);
            ...
            // READ ALL ATTRIBUTES
            staticReadCurrentPoint(stream,
                                   x, y, z,
                                   intensitiesV->tAt(i),
                                   mask05, mask610,
                                   classificationsV->tAt(i),
                                   scanAngleRank05, scanAngleRank610,
                                   userDataV->tAt(i),
                                   pointSourceIDV->tAt(i),
                                   gpsTime,
                                   red, green, blue,
                                   nir,
                                   wpdi, botwd, wpsib, rpwl);
