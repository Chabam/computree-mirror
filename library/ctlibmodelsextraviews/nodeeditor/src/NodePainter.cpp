                     ...
                     FlowScene const & scene)
{
  ...
  for(PortType portType: {PortType::Out, PortType::In})
  {
    ...
    for (unsigned int i = 0; i < n; ++i)
    {
      ...
      if(dataType.id.isEmpty()) // MK : skip connection point that can not be connected
          continue;

      QPointF p = geom.portScenePosition(i, portType);

      // MK : modification to enable in port accept multiple connection
      const bool canConnect = /*(state.getEntries(portType)[i].empty() ||
                         (portType == PortType::Out &&
                          model->portOutConnectionPolicy(i) == NodeDataModel::ConnectionPolicy::Many) )*/true;
                           ...
                           NodeDataModel const * model)
{
  ...
  for(PortType portType: {PortType::Out, PortType::In})
  {
    ...
    {
      QPointF p = geom.portScenePosition(PortIndex(i), portType);

      if (!state.getEntries(portType)[i].empty())
      {
        auto const & dataType = model->dataType(portType, PortIndex(i));
